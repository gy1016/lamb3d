import { Vector3, Matrix4, Quaternion, MathUtil, Matrix3 } from '@lamb3d/math';
...
export class Transform {
  ...
  private static _tempMat30: Matrix3 = new Matrix3();
  private static _tempMat31: Matrix3 = new Matrix3();
  private static _tempMat32: Matrix3 = new Matrix3();
  private static _tempMat41: Matrix4 = new Matrix4();
  private static _tempMat42: Matrix4 = new Matrix4();
  ...
  get worldMatrix(): Matrix4 {
    if (this._isContainDirtyFlag(TransformFlag.WorldMatrix)) {
      ...
      const parent = null;
      if (parent) {
        Matrix4.multiply(parent.worldMatrix, this.localMatrix, this._worldMatrix);
      } else {
  ...
  set worldMatrix(value: Matrix4) {
    ...
    const parent = null;
    if (parent) {
      Matrix4.invert(parent.worldMatrix, Transform._tempMat42);
      Matrix4.multiply(value, Transform._tempMat42, this._localMatrix);
    } else {
      value.cloneTo(this._localMatrix);
    }
  ...
  private _onWorldPositionChanged(): void {
    ...
    const parent = null;
    if (parent) {
      Matrix4.invert(parent.worldMatrix, Transform._tempMat41);
